...
import { resetPublishApartmentAction } from '../../../../features/publishApartmentSlice';
...
function Finalization(props) {
    ...
    const publishApartmentOnClick = () => {
        ...
        publishApartmentOnDB(userDataState.token, publishApartmentData.apartment)
        .then((res) => {
            setIsLoading(true);
            uploadApartmentFilesOnDB(userDataState.token, res, publishApartmentData.apartment.filesFd)
            .then((result) => {
                dispatch(resetPublishApartmentAction());
                history.push('/');
            })
            .catch((error) => {
                console.log(error);
